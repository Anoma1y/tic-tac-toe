...
class TicTacToe extends Component {
  ...
  createGameSession = () => {
    let sessionCode = this.random(1000, 9999);
  ...
  SubmitName = (e) => {
  	...
  	if (this.state.playerName.length >= 1) {
    	this.setState({ checkNameSubmited: true })
  	}
    
  }
  //Метод для ввода номера сессии
  ChangeJoin = event => {
    this.setState({ sessionCode: event.target.value });
  }
...
*/}            {buttonCreate}
    ...
    if(this.state.sessionCode !== '' && !this.state.start) {
      // renderCreateGameSession = undefined;
      renderGameSession =
        <div>
    ...
    //Рендер игрового поля, доступный после старта игры (когда подключенны оба игрока)
  	...
  	if (this.state.start) {
  		...
  		<div className="gameBoard">
  			...
  			{win}
  			<h3>Очередь игрока: <span></span></h3>
  			<Board 
  				socket={this.socket}
   				sessionCode={this.state.sessionCode} 
  			/>
